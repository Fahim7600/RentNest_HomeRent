...
import { fetchAdminRentals } from "../actions";
import {
  FileText,
  Calendar,
  Clock,
...
export function AdminRequestList() {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    ...
    queryFn: () => fetchAdminRentals({ status: statusFilter, page, limit: 10 }),
  });
    ...
    <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6 shadow-xl backdrop-blur-md space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-slate-800 pb-5">
        <div>
          <h2 className="flex items-center gap-2 text-lg font-bold text-white">
            ...
            Platform Rental Request Oversight
          </h2>
          <p className="mt-1 text-xs text-slate-400">
            Read-only audit view of all tenant rental applications and agreement statuses.
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-xl border border-red-500/20 bg-red-500/10 p-6 text-center">
          <p className="text-xs text-red-400">Failed to load platform rental requests.</p>
          <button
            onClick={() => refetch()}
