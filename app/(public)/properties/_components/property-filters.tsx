...
import { MapPin, DollarSign, Filter, RotateCcw, Tag, X, SlidersHorizontal } from "lucide-react";
...
export function PropertyFilters() {
  ...
  const activeFilterCount = [location, minPrice, maxPrice, categoryId].filter(Boolean).length;
    ...
    <>
      ...
      <div className="md:hidden">
        <button
          ...
          className="flex w-full items-center justify-between rounded-xl border border-slate-800 bg-slate-900/80 px-4 py-3 text-sm font-semibold text-white shadow-lg backdrop-blur-md"
        >
      ...
      <div className="hidden md:block rounded-2xl border border-slate-800 bg-slate-900/60 p-6 shadow-xl backdrop-blur-md">
      ...
      {isMobileOpen && (
        <div className="fixed inset-0 z-50 flex flex-col justify-end bg-slate-950/80 backdrop-blur-sm md:hidden">
          <div className="relative max-h-[85vh] w-full overflow-y-auto rounded-t-3xl border-t border-slate-800 bg-slate-900 p-6 shadow-2xl space-y-6">
            <div className="flex items-center justify-between border-b border-slate-800 pb-4">
              ...
              <div className="flex items-center gap-3">
                {activeFilterCount > 0 && (
                  <button
                    onClick={handleClear}
                    className="flex items-center gap-1 text-xs text-slate-400 hover:text-indigo-300"
                  >
                    <RotateCcw className="h-3 w-3" />
                    Reset
                  </button>
            ...
            <button
              onClick={() => setIsMobileOpen(false)}
              className="w-full rounded-xl bg-indigo-600 py-3 text-sm font-semibold text-white shadow-lg hover:bg-indigo-500 transition-colors"
            >
              Apply Filters
            </button>
          </div>
